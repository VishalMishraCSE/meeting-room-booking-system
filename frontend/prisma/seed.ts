...
async function main() {
  ...
  // 6. Create Initial Mock Bookings
  console.log('📅 Seeding initial bookings...');
  const baseDate = new Date();
  baseDate.setHours(13, 0, 0, 0); // 1:00 PM today
  ...
  await prisma.booking.create({
    data: {
      ...
      agenda: 'Reviewing backlog items for the upcoming sprint.',
      status: 'Confirmed',
      attendees: {
        create: [
          { email: employees[0].email, status: 'Accepted' },
          { email: managers[0].email, status: 'Pending' },
        ],
      },
      history: {
        create: [
          { action: 'Created', performedBy: employees[0].email },
        ],
      },
    },
  });
  ...
  await prisma.booking.create({
    data: {
      ...
      roomId: alphaBoardroom.id,
      startTime: new Date(baseDate.getTime() + 30 * 60 * 1000), // 1:30 PM
      endTime: new Date(baseDate.getTime() + 60 * 60 * 1000), // 2:00 PM
      title: 'Client Sync',
      agenda: 'Strategic sync call with core clients.',
      status: 'Confirmed',
      attendees: {
        create: [
          { email: managers[0].email, status: 'Accepted' },
        ],
      },
      history: {
        create: [
          { action: 'Created', performedBy: managers[0].email },
        ],
      },
    },
  });
  ...
  await prisma.booking.create({
    data: {
      ...
      roomId: betaLab.id,
      startTime: baseDate,
      endTime: new Date(baseDate.getTime() + 60 * 60 * 1000), // 1:00 PM to 2:00 PM
      title: 'Design Review',
      agenda: 'UX review for the room booking screens.',
      status: 'Confirmed',
      attendees: {
        create: [
          { email: employees[0].email, status: 'Accepted' },
